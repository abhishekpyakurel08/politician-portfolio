...
export function BlueHeroSlider({ 
  ...
  const defaultSlides = [
    {
      ...
      mediaUrl: "/website-template-OG.webp",
  ...
  const slides = initialSlide 
    ? [
        { 
          title: initialSlide.title || defaultSlides[0].title, 
          description: initialSlide.description || defaultSlides[0].description,
          mediaUrl: (typeof initialSlide.media === 'object' && initialSlide.media?.url) ? initialSlide.media.url : defaultSlides[0].mediaUrl,
  ...
  const next = () => { setIndex((prev) => (prev + 1) % slides.length); setProgress(0); }
  const prev = () => { setIndex((prev) => (prev - 1 + slides.length) % slides.length); setProgress(0); }
    ...
    <section className="relative w-full h-[75vh] md:h-[90vh] bg-slate-950 overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            ...
            <div className={cn("absolute inset-0 opacity-20", index === 1 ? 'bg-blue-900' : index === 2 ? 'bg-slate-900' : 'bg-red-900')} />
          </motion.div>
          ...
          <div className="absolute inset-0 flex items-center">
            <div className="container mx-auto max-w-[1240px] px-4 xl:px-8">
              <div className="max-w-4xl space-y-8">
                ...
                >
                  <Badge className={cn("text-white px-5 py-2 text-sm border-none rounded-full shadow-2xl", slides[index].accent)}>
                     {slides[index].badge}
                  </Badge>
                </motion.div>
                ...
                >
                  <Button size="lg" className="bg-[#B31B20] hover:bg-red-700 text-white font-black h-14 px-8 rounded-2xl shadow-2xl shadow-red-900/40 gap-3 group/btn text-base">
                    थप जानकारी <ChevronRight className="w-5 h-5 group-hover/btn:translate-x-2 transition-transform" />
                  </Button>
                  ...
                  <button className="flex items-center gap-5 group/play outline-hidden">
                    ...
                    <span className="text-white text-base font-black tracking-wider group-hover/play:text-red-400 transition-colors uppercase">{t.videoMessage}</span>
                  </button>
                </motion.div>
              </div>
            </div>
      ...
      <div className="absolute bottom-12 right-8 md:right-16 z-30 flex flex-col items-end gap-10">
        ...
        <div className="flex items-center gap-4">
          <button onClick={prev} className="w-14 h-14 rounded-2xl bg-white/5 backdrop-blur-2xl border border-white/10 flex items-center justify-center text-white hover:bg-[#B31B20] hover:scale-110 transition-all active:scale-95">
             <ChevronLeft className="w-6 h-6" />
          </button>
          <button onClick={next} className="w-14 h-14 rounded-2xl bg-[#B31B20] border border-white/10 flex items-center justify-center text-white hover:bg-red-700 hover:scale-110 transition-all active:scale-95 shadow-2xl shadow-red-900/50">
      ...
      <div className="absolute inset-x-0 bottom-0 px-8 py-4 z-40 bg-linear-to-t from-slate-950/80 to-transparent flex justify-between items-end pointer-events-none">
        <div className="h-1 bg-[#B31B20] transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>
