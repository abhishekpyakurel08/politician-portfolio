        ...
        {!selectedAlbum ? (
          ...
          >
            ...
            <div className="bg-[#B31B20] text-white p-6 md:p-8 flex items-center justify-between sticky top-0 z-10 shadow-xl">
              <div>
                <h2 className="text-2xl md:text-3xl font-black tracking-tighter uppercase">{selectedAlbum.title}</h2>
              ...
              <Button 
                variant="ghost" 
                size="icon" 
                className="text-white hover:bg-white/20 rounded-2xl w-12 h-12 transition-all hover:rotate-90 duration-300"
            ...
            <div className="p-6 md:p-10 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                >
                  <Image src={selectedAlbum.imageUrl} fill alt="Cover" className="object-cover group-hover:scale-110 transition-transform duration-1000" />
                ...
                {selectedAlbum.images.map((img, i) => (
                  ...
                  >
                    <Image src={img.url} fill alt={img.alt || `Photo ${i+1}`} className="object-cover group-hover:scale-125 transition-transform duration-1000" />
                    <div className="absolute inset-0 bg-linear-to-t from-black/20 to-transparent transition-opacity opacity-0 group-hover:opacity-100" />
                  </motion.div>
                ))}
              </div>
