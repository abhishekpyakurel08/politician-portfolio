...
export default async function HomePage() {
  ...
  const mapDocToCard = (doc: any) => ({
    ...
    imageUrl: (typeof doc.featuredImage === 'object' && doc.featuredImage?.url) ? (doc.featuredImage.url as string) : '/website-template-OG.webp',
  ...
  // Fallback Mock Data
  const mockNewsCard = {
    title: 'पार्टी स्थापना दिवसको अवसरमा शुभकामना सन्देश',
    excerpt: 'नेपाल कम्युनिष्ट पार्टीको स्थापना दिवसको अवसरमा आम नेपाली जनतामा हार्दिक शुभकामना व्यक्त गर्दछु।',
  ...
  const getDoc = (index: number) => dynamicNews[index] || { ...mockNewsCard, title: `गतिविधि ${index + 1}` }
  ...
  const galleryItems = galleryDocs.map(doc => {
    ...
    return {
      title: doc.title,
      slug: doc.slug,
      coverImg: photosArray.length > 0 && typeof photosArray[0].image === 'object' ? (photosArray[0].image.url as string) : '/website-template-OG.webp',
      subImg1: photosArray.length > 1 && typeof photosArray[1].image === 'object' ? (photosArray[1].image.url as string) : '/website-template-OG.webp',
      subImg2: photosArray.length > 2 && typeof photosArray[2].image === 'object' ? (photosArray[2].image.url as string) : '/website-template-OG.webp',
  ...
  const videoItems = videosDocs.map(doc => ({
    ...
    thumb: typeof doc.thumbnail === 'object' && doc.thumbnail?.url ? (doc.thumbnail.url as string) : '/website-template-OG.webp'
  }))
  const mainVideo = videoItems[0] || { title: 'मुख्य भिडियो', thumb: '/website-template-OG.webp' }
  ...
  return (
    <div className="relative overflow-hidden w-full">
      ...
      <section className="w-full py-16 md:py-32 bg-transparent relative z-10">
        <div className="container">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16">
             {/* Left - Hero Card */}
             <Link href={`/news/${getDoc(0).slug}`} className="group block h-full">
                <Card className="overflow-hidden border-none glass hover:shadow-red-900/20 transition-all duration-700 h-full flex flex-col rounded-3xl md:rounded-[40px] premium-border hover:-translate-y-2">
                  ...
                  <CardHeader className="p-5 md:p-8 flex-1">
                    <div className="flex items-center gap-2 text-slate-500 text-xs md:text-sm font-medium mb-2 md:mb-3">
                      ...
                      <span className="mukta-semibold">{getDoc(0).date}</span>
                    </div>
                    ...
                    <p className="mt-3 md:mt-4 text-slate-600 line-clamp-2 md:line-clamp-3 text-sm md:text-base leading-relaxed">
                      {getDoc(0).excerpt || "जनताको जनमुक्ति आन्दोलन र क्रान्तिको इतिहासलाई सम्मान गर्दै अगाडि बढ्ने हाम्रो संकल्प..."}
                    </p>
                  </CardHeader>
                  <CardFooter className="px-5 pb-8 md:px-10 md:pb-12 pt-0">
                    <Button variant="ghost" className="p-0 h-auto text-[#B31B20] font-black hover:bg-transparent hover:text-red-700 gap-3 text-base md:text-lg group/more">
                      थप पढ्नुहोस् <ChevronRight className="w-5 h-5 md:w-6 md:h-6 group-hover/more:translate-x-2 transition-transform duration-300" />
                    </Button>
                  </CardFooter>
                </Card>
             </Link>
                 ...
                 <Link key={idx} href={`/news/${getDoc(idx).slug}`} className="group">
                   <Card className="overflow-hidden border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 rounded-xl md:rounded-2xl">
                     <div className="flex flex-row h-full">
                       <div className="relative w-[100px] sm:w-[200px] shrink-0">
                         <AspectRatio ratio={4 / 3}>
                           <Image 
                             src={getDoc(idx).imageUrl} 
                             alt={getDoc(idx).title} 
                             fill 
                       ...
                       <CardHeader className="p-3 md:p-4 flex flex-col justify-center flex-1">
                          <Badge variant="outline" className="w-fit mb-1 md:mb-2 text-[#B31B20] border-red-100 bg-red-50/50 text-[10px] md:text-xs px-2 py-0.5">
      ...
      <section className="w-full bg-slate-950 py-16 md:py-24 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-red-600/10 blur-[120px] -mr-40 -mt-40" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-blue-600/10 blur-[100px] -ml-20 -mb-20" />
        
        <div className="container relative z-10">
           <SectionHeading title="दृश्यमा परिवर्तन" viewAllHref="/videos" textColor="text-white" className="border-slate-800" />
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
              ...
              <div className="lg:col-span-8 group relative rounded-3xl overflow-hidden shadow-2xl ring-1 ring-white/10 premium-border">
                <AspectRatio ratio={16 / 9}>
                   <Image src={mainVideo.thumb} alt={mainVideo.title} fill className="object-cover brightness-[0.6] group-hover:brightness-75 transition-all duration-700 group-hover:scale-105" sizes="(max-width: 1024px) 100vw, 66vw" />
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-6 md:p-12 bg-linear-to-t from-black text-white">
                     <Badge className="bg-[#B31B20] mb-3 text-[10px] md:text-sm border-none px-3 py-1 font-black mukta-bold">विशेष सामाग्री</Badge>
              ...
              <div className="lg:col-span-4 flex flex-col gap-4 md:gap-5">
                <div className="glass-dark border-white/10 p-6 md:p-8 rounded-3xl hidden md:block premium-border">
                   <h3 className="font-black text-xl md:text-2xl text-white leading-tight mukta-bold text-gradient-hero">तपाईंलाई थाहा छ?</h3>
                   <p className="text-slate-400 mt-3 text-sm md:text-lg leading-relaxed font-medium">विकास र समृद्धिका यी पाइलाहरू कसरी अगाडि बढिरहेका छन्? विस्तृतमा हेर्नुहोस्।</p>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                  {subVideos.map((vid, i) => (
                    <div key={i} className="group relative flex gap-3 md:gap-4 items-center bg-white/5 hover:bg-white/10 backdrop-blur-xs border border-white/10 rounded-xl p-2.5 md:p-3 transition-all duration-300 cursor-pointer">
                      <div className="relative w-24 md:w-32 shrink-0 rounded-lg overflow-hidden">
                        <AspectRatio ratio={16/9}>
                          <Image src={vid.thumb} alt={vid.title} fill className="object-cover scale-105 group-hover:scale-125 transition-transform duration-700" />
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-sm md:text-base text-slate-100 line-clamp-2 leading-snug group-hover:text-red-400 transition-colors">{vid.title}</h4>
                        <div className="flex gap-2 mt-2 md:mt-3">
                           <Button size="icon" variant="ghost" className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white transition-all">
                             <Facebook className="w-3 md:w-3.5 h-3 md:h-3.5"/>
                           </Button>
                           <Button size="icon" variant="ghost" className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-sky-500/20 hover:bg-sky-500 text-sky-400 hover:text-white transition-all">
         ...
         <div className="container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mt-8 md:mt-16">
              {galleryItems.map((album, i) => (
                <Link key={i} href={`/gallery`} className="group flex flex-col gap-4 md:gap-6 bg-white p-4 md:p-6 rounded-[32px] md:rounded-[48px] shadow-sm hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] transition-all duration-700 ring-1 ring-slate-100 hover:-translate-y-3">
                  <div className="grid grid-cols-2 gap-2 md:gap-3 h-[240px] md:h-[400px]">
                     <div className="relative col-span-1 h-full rounded-xl md:rounded-2xl overflow-hidden shadow-inner bg-slate-100">
                        <Image src={album.coverImg} alt="Gallery cover" fill className="object-cover group-hover:scale-105 transition-transform duration-1000" />
                     </div>
                     <div className="grid grid-rows-2 gap-2 md:gap-3 h-full">
                        <div className="relative rounded-xl md:rounded-2xl overflow-hidden shadow-inner bg-slate-100">
                           <Image src={album.subImg1} alt="sub image 1" fill className="object-cover" />
                        </div>
                        <div className="relative rounded-xl md:rounded-2xl overflow-hidden shadow-inner bg-slate-100">
                           <Image src={album.subImg2} alt="sub image 2" fill className="object-cover" />
                           <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] flex flex-col items-center justify-center text-white">
                              ...
                              <span className="text-[10px] md:text-xs font-bold tracking-widest uppercase">Photos</span>
                           </div>
                        </div>
                     </div>
                  </div>
                  <div className="px-2 md:px-4 py-2">
                    <h3 className="text-xl md:text-3xl font-black text-slate-900 line-clamp-1 group-hover:text-[#B31B20] transition-colors mukta-bold tracking-tighter">{album.title}</h3>
                    <p className="text-slate-500 font-black text-[10px] md:text-xs mt-1 md:mt-2 uppercase tracking-[0.2em]">ग्यालरी अपडेट</p>
                  </div>
                </Link>
              ))}
            </div>
         ...
         <div className="container">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-12">
                  ...
                  <div className="md:w-1/2 group">
                      <Card className="overflow-hidden border-none shadow-xl h-full rounded-2xl md:rounded-3xl">
                         <div className="relative aspect-4/5 overflow-hidden">
                           <Image src={getDoc(14).imageUrl} alt={getDoc(14).title} fill className="object-cover group-hover:scale-110 transition-transform duration-1000" />
                           <div className="absolute inset-0 bg-linear-to-t from-black via-transparent to-transparent" />
                           <div className="absolute inset-x-0 bottom-0 p-5 md:p-8">
                              <Badge className="bg-red-600 mb-3 md:mb-4 px-2 md:px-3 py-0.5 md:py-1 text-[10px] md:text-xs border-none">विशेष निर्णय</Badge>
                              <h3 className="text-white font-black text-xl md:text-3xl leading-tight group-hover:text-red-400 transition-colors">{getDoc(14).title}</h3>
                           </div>
                         </div>
                      </Card>
                  </div>
                       ...
                       <Link key={idx} href={`/news/${getDoc(idx).slug}`} className="group">
                         <Card className="p-1 border-slate-100 hover:border-red-100 bg-slate-50/30 hover:bg-white shadow-sm hover:shadow-lg transition-all duration-300 rounded-xl md:rounded-2xl">
                           <div className="relative aspect-video overflow-hidden rounded-lg mb-3 md:mb-4">
                             <Image src={getDoc(idx).imageUrl} alt={getDoc(idx).title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                           </div>
              ...
              <div className="lg:col-span-1 flex flex-col gap-8 md:gap-10">
                ...
                {/* Press Releases */}
                 <Card className="border-none glass shadow-inner overflow-hidden rounded-[32px] premium-border">
                    <CardHeader className="bg-slate-900 py-6 px-8 md:py-8 md:px-10">
                       <div className="flex justify-between items-center">
                         <SectionHeading title="प्रेस विज्ञप्ति" textColor="text-white" className="border-none py-0 mb-0 scale-90 -ml-4" />
                       </div>
                    </CardHeader>
                   <CardContent className="p-0">
                      <div className="flex flex-col">
                        {[18, 19, 11, 12, 13].map((idx, i) => (
                          <Link key={idx} href="#" className="flex items-center gap-4 md:gap-6 p-4 md:p-6 border-b border-slate-200/50 bg-white hover:bg-slate-50 group transition-all">
                            <span className="text-slate-200 font-black text-4xl md:text-6xl shrink-0 tabular-nums group-hover:text-red-100 transition-colors mukta-bold">0{i+1}</span>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <Badge variant="secondary" className="bg-slate-100 text-slate-500 text-[10px] md:text-xs font-black uppercase flex items-center gap-1.5 group-hover:bg-red-50 group-hover:text-red-600 transition-colors px-2 py-0.5">
                ...
                <Card className="border-slate-100 shadow-xl overflow-hidden p-1 rounded-2xl">
                   <div className="p-3 md:p-4 bg-slate-900 flex justify-between items-center rounded-t-xl">
                      ...
                      <Link href="#" className="text-white/60 hover:text-white text-[10px] md:text-xs font-bold">FOLLOW UP</Link>
                   </div>
                   <div className="w-full h-[400px] md:h-[500px] bg-slate-50/50 flex justify-center">
                      <iframe 
      ...
      <section className="w-full bg-[#1A365D] text-white py-24 md:py-32 relative overflow-hidden flex items-center justify-center border-t-4 md:border-t-8 border-[#B31B20]">
         ...
         <div className="relative z-10 container flex flex-col items-center justify-center text-center max-w-5xl">
            <div className="relative group mb-6 md:mb-10">
               ...
               <div className="w-20 h-20 md:w-32 md:h-32 rounded-full overflow-hidden ring-2 md:ring-4 ring-white shadow-2xl relative z-10">
                   <Image src="/website-template-OG.webp" alt="Leader" fill className="object-cover" sizes="128px" />
               </div>
            </div>
            ...
            <div className="mt-6 md:mt-8 flex flex-col items-center">
               ...
               <h4 className="text-lg md:text-xl font-bold tracking-[0.2em] text-red-500 uppercase">पार्टी अध्यक्ष</h4>
            </div>
         </div>
      </section>
