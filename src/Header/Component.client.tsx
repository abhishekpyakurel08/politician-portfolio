  ...
  return (
    <header 
      className={cn(
        ...
        isScrolled 
          ? "py-2 glass shadow-2xl h-16 md:h-20" 
    ...
    >
      ...
      <div className="container mx-auto max-w-[1240px] px-6 h-full flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-2">
              ...
              return (
                <Link 
                  ...
                  className={cn(
                    ...
                    isScrolled
                      ? isActive ? "text-[#B31B20]" : "text-slate-600 hover:text-[#B31B20]"
                  ...
                  {isActive && (
                    <motion.div 
                      ...
                      className={cn(
                        ...
                        isScrolled ? "bg-[#B31B20]/5" : "bg-white/10"
                      )}
          ...
          <div className={cn("h-8 w-px mx-4 transition-colors", isScrolled ? "bg-slate-200" : "bg-white/20")} />
          ...
          <div className="flex items-center gap-3">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white border-none shadow-2xl rounded-2xl p-2 min-w-[160px]">
                <DropdownMenuItem onClick={() => setLocale('ne')} className="font-bold py-3 px-4 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">नेपाली (NP)</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setLocale('en')} className="font-bold py-3 px-4 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">English (UK)</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <Button asChild variant="ghost" size="icon" className={cn(
              ...
              isScrolled 
                ? "bg-slate-100 text-slate-700 hover:bg-[#B31B20] hover:text-white shadow-sm" 
        ...
        <div className="lg:hidden flex items-center gap-3">
          <Button asChild variant="ghost" size="icon" className={cn(
              ...
              isScrolled ? "text-slate-700 bg-slate-100" : "text-white bg-white/10"
            )}>
            <Link href="/search">
              <Search className="w-5 h-5" />
              <span className="sr-only">Search</span>
          ...
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className={isScrolled ? "text-slate-700" : "text-white"}>
            ...
            <SheetContent side="right" className="w-full sm:w-[400px] border-none glass-dark text-white p-0">
              ...
              <div className="mt-auto p-10 flex flex-col gap-8">
                ...
                <Button className="w-full bg-white text-slate-950 font-black h-16 rounded-2xl hover:bg-[#B31B20] hover:text-white transition-all text-sm uppercase tracking-widest shadow-2xl">
                   Switch to नेपाली (NP)
                </Button>
              </div>
            </SheetContent>
